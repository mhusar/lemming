...
InputPanel = (function () {
    ...
    var documentIdentifier = "",
        ...
        insertCharacter = function (formElement, character,
            ...
            if (formElement instanceof jQuery) {
                ...
                if (selectionStart <= selectionEnd) {
                    formElement.val(value.substring(0, selectionStart)
                        + character + value.substring(selectionEnd));
                    formElement[0].setSelectionRange(selectionStart + 1, selectionStart + 1);
                } else {
                    formElement.val(value.substring(0, selectionEnd)
                        + character + value.substring(selectionStart));
        ...
        initCharacterElements = function () {
            var charactersArray = jQuery("body").data("characters"),
                lastFocusedElement,
                selectionStart,
                selectionEnd;
            ...
            jQuery(".inputPanel .characters")
                .on(
                    ...
                    function () {
                        ...
                        /*jslint unparam: true*/
                        jQuery(".inputPanel .characters .character")
                    ...
                    function () {
                        ...
                        if (lastFocusedElement.length) {
                            ...
                            if (editor !== undefined) {
                                ...
                                insertCharacter(lastFocusedElement, character,
                                    selectionStart, selectionEnd);
                            }
                        }
                    })
        ...
        insertTag = function (formElement, tagObject,
            selectionStart, selectionEnd) {
            var cursorPosition,
                insertString,
                selectionRange,
            ...
            if (formElement instanceof jQuery) {
                ...
                if (selectionStart <= selectionEnd) {
                    insertString = (tagObject.selfClosing) ? "<" + tagObject.name + "/>"
                            : "<" + tagObject.name + ">"
                    ...
                    formElement.val(value.substring(0, selectionStart)
                        + insertString + value.substring(selectionEnd));
                } else {
                    insertString = (tagObject.selfClosing) ? "<" + tagObject.name + "/>"
                            : "<" + tagObject.name + ">"
                            + value.substring(selectionEnd, selectionStart)
                            + "</" + tagObject.name + ">";
                    cursorPosition = selectionEnd + insertString.length;

                    formElement.val(value.substring(0, selectionEnd)
                        + insertString + value.substring(selectionStart));
                }
                ...
                insertString = (tagObject.selfClosing) ? "<" + tagObject.name + "/>"
                        : "<" + tagObject.name + ">" + selectionString
                        + "</" + tagObject.name + ">";
        ...
        initTagElements = function () {
            ...
            jQuery("main")
                ...
                .on(
                    ...
                    "textarea",
                    function () {
                        var container = jQuery(this).parents(".xmlEditor"),
                            documentObject;
                ...
                .on(
                    "blur",
                    "input[type=number], input:text, textarea",
                    function () {
                        var that = this;
            ...
            jQuery(".inputPanel .tags")
                .on("click",
                    ".tag",
                    function () {
                        var tagObject = jQuery(this).data("tag"),
                            editor;
                        ...
                        if (lastFocusedElement.length) {
                            ...
                            if (editor !== undefined) {
                                ...
                                if (tagObject.attributes.length) {
                                    showTagDialog(lastFocusedElement, tagObject,
                                        selectionStart, selectionEnd);
                                } else {
                                    insertTag(lastFocusedElement, tagObject,
                                        selectionStart, selectionEnd);
                                }
                            }
                        }
...
TagDialog = (function () {
    ...
    var addAttributes = function (tagObject) {
            var attributeObject,
                ...
                formGroup,
                i,
                tabDialogBody = jQuery(".tab-dialog .modal-body");

            for (i = 0; i < tagObject.attributes.length; i = i + 1) {
                attributeObject = tagObject.attributes[i];
                formControl = jQuery("<input type='text'/>")
                    .addClass("form-control").attr("data-attribute", attributeObject.name);
                formGroup = jQuery("<div></div>").addClass("form-group");

                tabDialogBody.append(jQuery("<div></div>").addClass("row")
                    .append(jQuery("<div></div>").addClass("col-md-12")
                        .append(formGroup)));
                formGroup.append(jQuery("<label></label>")
                    .addClass("control-label col-sm-3").text(attributeObject.name));
                formGroup.append(jQuery("<div></div>").addClass("col-sm-9")
                    .append(formControl));
        ...
        addTitle = function (tagObject) {
            var tabDialogTitle = jQuery(".tab-dialog .modal-title"),
                titleStart =  tabDialogTitle.data("title-start"),
        ...
        insertTag = function (formElement, tagObject, selectionStart, selectionEnd) {
            var attributeString = createAttributeString(),
                cursorPosition,
                insertString,
                selectionRange,
                selectionString,
            ...
            if (formElement instanceof jQuery) {
                ...
                if (selectionStart <= selectionEnd) {
                    insertString = (tagObject.selfClosing)
                        ? "<" + tagObject.name + attributeString + "/>"
                        : "<" + tagObject.name + attributeString + ">"
                            + value.substring(selectionStart, selectionEnd)
                            + "</" + tagObject.name + ">";
                    cursorPosition = selectionStart + insertString.length;
        ...
        create = function (tagObject) {
            var body = jQuery("body"),
                tabDialogTemplate = jQuery(".tab-dialog-template");
